      ...
      <div class="area_ctrl" v-if="show">
        ...
        <div class="area_roll_mask">
          <div class="area_roll">
            ...
            <div class="ares_line" v-if="unit=='kg'"><div class="area_grid">
            </div><span></span></div>
    ...
    setGear(target, val) {
      ...
      if (!this.selectData.link) {
        if (type === 'provs') {
          ...
          _self.$emit('chooseOne', _self.selects)
        } else if (type === 'city') {
          _self.selects.select2 = _self.pData2[endVal]
          _self.$emit('chooseTwo', _self.selects)
        } else {
          _self.selects.select3 = _self.pData3[endVal]
          _self.$emit('chooseThr', _self.selects)
        }
      } else {
        if (type === 'provs') {
          _self.selects.select1 = _self.pData1[endVal]
  ...
  watch: {
    selectData: {
      handler: function (val) {
        var that=this
        var county = that.$refs.county;
        var city =that.$refs.city;
        that.init()
        if(that.ajaxOnOff &&  county.style!=='undefined'){        //后续优化，先归0
          county.setAttribute('top', 0);
          county.style["-webkit-transform"] = 'translate3d(0,' + 0 + 'em,0)';
        }
        if(that.addressOnoff && city.style!=='undefined'){       //处理数据长短问题,默认第一个
          city.setAttribute('top', 0);
          city.style["-webkit-transform"] = 'translate3d(0,' + 0 + 'em,0)';
        }
      },
      deep: true
    },
